...
from plans.master_plan_data import WEEKDAY_PATTERNS, halfMarathonPerformanceMasterPlan
...
def exercise_slug(name):
    return f"half_marathon_{slugify(name).replace('-', '_')}"[:128]
...
def nutrition_for(workout):
    ...
    return {
        "title": "Half marathon performance fueling",
        "description": "Fuel quality sessions and rehearse confident race execution.",
        "bullets": [
            "Use carbohydrates before threshold, VO2, HM-pace, and long-run sessions.",
            "Practice race fluids or gels during long runs." if long_run else "Keep hydration steady and protein consistent.",
            "Add electrolytes for hard, hot, or high-sweat work." if hard or long_run else "Keep recovery meals simple and consistent.",
        ],
    }


def seed_half_marathon_plan(apps, schema_editor):
    ...
    data = halfMarathonPerformanceMasterPlan
        ...
        for workout_data in week_data["workouts"]:
            ...
            for exercise_data in workout_data["exercises"]:
                ...
                if slug not in exercise_cache:
                    ...
                    if exercise is None:
                        ...
                        exercise = PlanExercise.objects.create(
                            ...
                            coaching_cues=[
                                "Stay relaxed while pace increases",
                                "Protect cadence under fatigue",
                                "Finish fast without forcing mechanics",
                            ],
                            common_mistakes=[
                                "Opening too fast",
                ...
                PlanDayExercise.objects.create(
                    ...
                    progression_rule=(
                        "Follow the authored half-marathon progression; do not add "
                        "extra work outside the selected rhythm."
                    ),
                )


def remove_half_marathon_plan(apps, schema_editor):
    Plan = apps.get_model("plans", "Plan")
    Plan.objects.filter(id=halfMarathonPerformanceMasterPlan["id"]).delete()


class Migration(migrations.Migration):

    dependencies = [
        ("plans", "0012_seed_full_marathon_elite_build"),
    ]

    operations = [
        migrations.RunPython(seed_half_marathon_plan, remove_half_marathon_plan),
    ]
